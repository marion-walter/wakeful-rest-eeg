...
def pg_compute_pre(df, predictor, outcome, test, subject=None, show = False):
    ...
    if test == 't-test_ind':
        groups = df[predictor].unique().tolist()
        ...
        res = pg.rm_anova(dv=outcome, within=predictor, data=df, detailed=False, effsize = 'ng2', subject = subject)
    ...
    if pval < 0.05:
        ...
        results = {'p':pval, 'es':None, 'es_label':None, 'es_interp':None}
...
def es_interpretation(es_label , es_value):
    ...
    elif es_label == 'np2' or es_label == 'ng2':
        if es_value < 0.01:
            interpretation = 'VS'
...
def auto_stats(df, 
    ...
    fontsize_stars = 20 # fontsize of pvalue stars
    ...
    if type(predictor) is str: # one way
        ...
        # sns.boxplot(x=predictor, y=outcome, data=df, order=order, width=0.2, palette=palette, ax=ax, showfliers = False) # add boxplot on top of violinplot
        # sns.catplot(x=predictor, y=outcome, data=df, order=order, capsize=.2, palette=palette, errorbar="se",kind="point", height=6, aspect=.75, ax=ax)
            ...
            if not post_test is None: # loop over pairwise combinations to plot annotations
                ...
                for i, row in df_annot.iterrows():
                    ...
                    if row['p-corr'] < 0.05 or show_ns:
                        ...
                        y = [row['y']] * len(x)
                        ax.plot(x,y , color = 'k', ls = ls, label='_sig_bar') # horizontal bar  

                        x = [row['xstart'], row['xstart']]
                        y = [row['y'], row['y'] - sd / 15]
                        ax.plot(x,y , color = 'k', label='_sig_tick') # small left vertical bar

                        x = [row['xstop'], row['xstop']]
                        y = [row['y'], row['y'] - sd / 15]
                        ax.plot(x,y , color = 'k', label='_sig_tick') # small right vertical bar
                ...
                x = [0, 1]
                y_plot = [y] * len(x)
                ax.plot(x,y_plot , color = 'k', ls = ls,label='_sig_bar') # horizontal bar  

                x = [0, 0]
                y_plot = [y, y - sd / 10]
                ax.plot(x,y_plot , color = 'k', label='_sig_tick') # small left vertical bar

                x = [1, 1]
                y_plot = [y,y- sd / 10]
                ax.plot(x,y_plot , color = 'k', label='_sig_tick') # small right vertical bar
        ...
        if xtick_info:
            ...
            if parametricity:
                ...
                ticks_estimators = [f"{cond} \n {round(estimators.loc[cond,outcome], 2)} ({round(estimators.loc[cond,'sd'], 2)})" for cond, ci in zip(order,cis)]
        ...
        if title_info == 'full':
            if design == 'between':
                if es_label is None:
                    ...
                    ax.set_title(f'Effect of {predictor} on {outcome_clean_label} : {pval_stars(pval)} \n {pre_test} : p{readable_p}, {es_label} : {es}', fontsize = title_fontsize)
            elif design == 'within':
                n_subjects = df[subject].unique().size
                if es_label is None:
        ...
        if strip is True:
            sns.stripplot(x=predictor, y=outcome, data=df, order=order , color = 'k', alpha = 0.5, size = 8, jitter = 0.02, ax=ax)
        ...
        if design == 'within':
            ...
            test_type = 'anova'
            test = pg.anova(data=df, dv=outcome, between = predictor).set_index('Source').round(2)
        ...
        for i, level in enumerate(df[hue_predictor].unique()):
            ...
            pval = res['p-unc'] * multiple_comparison_correction
            star = pval_stars(pval)
...
def lmm(df, predictor, outcome, subject, order=None):
    ...
    coefs = mdf.fe_params.to_frame(name = 'coef').round(2)
...
def confidence_interval(x, confidence = 0.95, verbose = False):
    ...
    if verbose:
        print(f'm : {round(m, 2)} , std : {round(s,2)} , ci : [{round(ci[0],2)};{round(ci[1],2)}]')
    return ci
...
def stats_quantitative(df, xlabel, ylabel, ax=None, corr_method = 'spearman', color_scatter = None, alpha_scatter = 0.6, legend_label = None, color_regline = 'r', return_ax_only = True):
    ...
    # ax.set_title(f'Correlation ({corr_method}) : {round(r, 2)}, p : {stars_corr}\nSlope : {round(slope, 2)} - R² : {round(rsquare, 3)}, p : {stars_reg}')
    ax.set_title(f'Correlation ({corr_method}) : {round(r, 2)}, p : {round(pval_corr, 2)} ({stars_corr})\nSlope : {round(slope, 2)} - R² : {round(rsquare, 2)}, p : {round(pval_reg, 2)} ({stars_reg})')
...
def stats_quali_quali(data, predictor, outcome, show = True, save = None):
    ...
    if show:
        ...
        suptitle = f'Effect of {predictor} on {outcome} : p = {round(p, 2)}'
