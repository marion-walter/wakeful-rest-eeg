...
def compute_spectrum(run_key, **p):
    ...
    **p : dict
    ...
    elif p == spectrum_params_replication:
    ...
    for condition in ['Repos', 'Distraction']:
        ...
        for base in subject_keys: 
            ...
            if replication_inspired == True:
                ...
                if compute_spectrum_replication_inspired_params_job.is_job_done(key):
                    subject_list.append(key)
            else:
                # print(f"Checking if job done for original params: {key}")
                if compute_spectrum_job.is_job_done(key):
                    subject_list.append(key)
        ...
        for subject in subject_list:
            if replication_inspired == True:
                ...
                ds = compute_spectrum_replication_inspired_params_job.get(subject)
            else:
                # print(f"Loading original spectrum for subject: {subject}")
                ds = compute_spectrum_job.get(subject)
        ...
        for order in ['both']:        
            ...
            for band in frequency_bands:
                        ...
                        for channel in df_o_d['Channel'].unique():
                            if channel in sub_chan :
                                ...
                                results.append({
                                    ...
                                    # ALL PSD METRICS (mean per channel) 
                    ...
                    if replication_inspired == True:
                        df_results.to_csv(figures_path / 'eeg_correlations_results_replication_inspired.csv', index=False)
                    if replication_inspired == False:
                        df_results.to_csv(figures_path / 'eeg_correlations_results.csv', index=False)
                    ...
                    if replication_inspired == True:
                        ...
                        if rank_data == True:
                            ...
                            if band == 'Slow (0.3 - 1 Hz)' and condition == 'Rest':
                                name = f"Figure_3A.png"
                                folder = figures_path
                            elif band == 'Alpha (8 - 12 Hz)' and condition == 'Rest':
                                name = f"Figure_4A.png"
                                folder = figures_path                               
                            else:
                                name = f"Order-{order}_{band}_Cond-{condition}_ranked.png"
                                # folder = figures_path / 'Analysis'  / 'Replication_inspired' / 'Ranked'
                                folder = figures_path / 'unused_figures'
                        else:
                            ...
                            if band == 'Slow (0.3 - 1 Hz)' and condition == 'Rest':
                                name = f"Supplementary_Figure_3A.png"
                                folder = figures_path
                            elif band == 'Alpha (8 - 12 Hz)' and condition == 'Rest':
                                name = f"Supplementary_Figure_4A.png"
                                folder = figures_path
                            else:
                                name = f"Order-{order}_{band}_Cond-{condition}.png"
                                # folder = figures_path / 'Analysis'  / 'Replication_inspired' / 'Non_ranked'
                                folder = figures_path / 'unused_figures'
                        ...
                        if rank_data == True:
                            ...
                            if band == 'Slow (0.3 - 1 Hz)' and condition == 'Rest':
                                name = f"Figure_3B.png"
                                folder = figures_path
                            elif band == 'Alpha (8 - 12 Hz)' and condition == 'Rest':
                                name = f"Figure_4B.png"
                                folder = figures_path 
                            else: 
                                name = f"Order-{order}_{band}_Cond-{condition}_ranked.png"
                                # folder = figures_path / 'Analysis'  / 'Our' / 'Ranked'
                                folder = figures_path / 'unused_figures'
                        else:
                            ...
                            if band == 'Slow (0.3 - 1 Hz)' and condition == 'Rest':
                                name = f"Supplementary_Figure_3B.png"
                                folder = figures_path
                            elif band == 'Alpha (8 - 12 Hz)' and condition == 'Rest':
                                name = f"Supplementary_Figure_4B.png"
                                folder = figures_path
                            else:
                                name = f"Order-{order}_{band}_Cond-{condition}.png"
                                # folder = figures_path / 'Analysis'  / 'Our' / 'Non_ranked'
                                folder = figures_path / 'unused_figures'
...
########################################################################
                    ...
                    if replication_inspired:
                        save_figure(fig, name, compute_spectrum_figure_replication_inspired_params_job, folder, plotly=False, with_job=True)
                        save_figure(fig, name, compute_spectrum_figure_replication_inspired_params_job, folder, plotly=False, with_job=False)
                    else:
                        save_figure(fig, name, compute_spectrum_figure_job, folder, plotly=False, with_job=True)
                        save_figure(fig, name, compute_spectrum_figure_job, folder, plotly=False, with_job=False)
...
def compute_spectrum_all():
    ...
    jobtools.compute_job_list(compute_spectrum_replication_inspired_params_job, run_keys, force_recompute=False, engine = 'loop')
    if spectrum_params_replication['rank_data'] == True:
        print("spectrum for replication-inspired analysis on ranked data computed")
    else:
        print("spectrum for replication-inspired analysis on real data computed")

    jobtools.compute_job_list(compute_spectrum_job, run_keys, force_recompute=False, engine = 'loop')
    if spectrum_params['rank_data'] == True:
        print("spectrum for our analysis on ranked data computed")
    else:
        print("spectrum for our analysis on real data computed")



def plot_correlation_EEG_memory(ranked_data, replication_inspired_analysis):
...
def test_compute_spectrum():
    run_key = 'sub-01_Repos'  
    ds = compute_spectrum(run_key, **spectrum_params)
    # ds = compute_spectrum(run_key, **spectrum_params_replication)

    print("done")
    print(ds)   


def test_compute_spectrum_figure():
    compute_spectrum_figure(**spectrum_params)
    print("done")



compute_spectrum_replication_inspired_params_job = jobtools.Job(precomputedir, 'compute_spectrum_replication_inspired', spectrum_params_replication, compute_spectrum)
jobtools.register_job(compute_spectrum_replication_inspired_params_job)

compute_spectrum_job = jobtools.Job(precomputedir, 'compute_spectrum', spectrum_params, compute_spectrum)
jobtools.register_job(compute_spectrum_job)

compute_spectrum_figure_replication_inspired_params_job = jobtools.Job(precomputedir, 'compute_spectrum_figure_replication_inspired', spectrum_params_replication, compute_spectrum_figure)
jobtools.register_job(compute_spectrum_figure_replication_inspired_params_job)

compute_spectrum_figure_job = jobtools.Job(precomputedir, 'compute_spectrum_figure', spectrum_params, compute_spectrum_figure)
jobtools.register_job(compute_spectrum_figure_job)
