...
from analyses_eeg import compute_spectrum, compute_spectrum_replication_inspired_params_job, compute_spectrum_job
        ...
        for order in ['both']:        
            ...
            for band in frequency_bands:
                    ...
                    if replication_inspired == True:
                        ...
                        # print(f"Columns in df_memory_order: {df_memory_order.columns}")
                    ...
                    if replication_inspired == True:
                        df_results.to_csv(figures_path / 'eeg_correlations_results_replication_inspired_reversed_memory.csv', index=False)
                    if replication_inspired == False:
                        df_results.to_csv(figures_path / 'eeg_correlations_results_reversed_memory.csv', index=False)
                    # Apply FDR correction to p_channels_uncorrected
                    _, p_channels_corrected = fdrcorrection(p_channels_uncorrected, alpha=0.05)
                    ...
                    if replication_inspired == False:
                        ...
                        if rank_data == True:
                            ...
                            if band == 'Slow (0.3 - 1 Hz)' and condition == 'Rest':
                                name = f"Supplementary_Figure_8B.png"
                                folder = figures_path 
                            else:
                                
                                name = f"Order-{order}_{band}_Cond-{condition}_ranked.png"
                                # folder = figures_path / 'Analysis'  / 'REVERSED' / 'reversed_memory'/ 'GMC_data_adated_analysis' / 'ranked'
                                folder = figures_path / 'unused_figures' / 'reversed_memory'/ 'GMC_data_adated_analysis' / 'ranked'
                        else:
                            ...
                            # folder = figures_path / 'Analysis'  / 'REVERSED' / 'reversed_memory'/ 'GMC_data_adated_analysis' / 'absolute'
                            folder = figures_path / 'unused_figures' / 'reversed_memory'/ 'GMC_data_adated_analysis' / 'absolute'
                    else:
                        if rank_data == True:
                            ...
                            name = f"Order-{order}_{band}_Cond-{condition}_ranked.png"
                            # folder = figures_path / 'Analysis'  / 'REVERSED' / 'reversed_memory'/ 'RMC_replication_inspired_analysis' / 'ranked'
                            folder = figures_path / 'unused_figures' / 'reversed_memory'/ 'RMC_replication_inspired_analysis' / 'ranked'
                        else:
                            ...
                            name = f"Order-{order}_{band}_Cond-{condition}.png"
                            # folder = figures_path / 'Analysis'  / 'REVERSED' / 'reversed_memory'/ 'RMC_replication_inspired_analysis' / 'absolute'
                            folder = figures_path / 'unused_figures' / 'reversed_memory'/ 'RMC_replication_inspired_analysis' / 'absolute'
                    ...
                    if replication_inspired:
                        print(
                            ...
                            f"Memory Score: Relative Memory Change\n"
                        ...
                        print(
                            ...
                            f"Frequency Band: {band}\n"
                            f"Replication Inspired: {replication_inspired}\n"
                            f"Memory Score: Global Memory Change\n"
                            f"Rank Data: {rank_data}\n"
                            f"Figure Name: {name}\n"
                            f"Folder: {folder}\n"
                            f"------------------------\n"
                    ...
                    if replication_inspired:
                        save_figure(fig, name, compute_spectrum_figure_replication_inspired_params_rev_mem_job, folder, plotly=False, with_job=True)
                        save_figure(fig, name, compute_spectrum_figure_replication_inspired_params_rev_mem_job, folder, plotly=False, with_job=False)
                    else:
                        save_figure(fig, name, compute_spectrum_figure_rev_mem_job, folder, plotly=False, with_job=True)
                        save_figure(fig, name, compute_spectrum_figure_rev_mem_job, folder, plotly=False, with_job=False)
                    plt.show()
                    plt.close(fig)
...
def plot_correlation_EEG_memory_reversed_memory_score(ranked_data, replication_inspired_analysis):
    ...
    if replication_inspired_analysis:
        print("\n--- Starting Replication-Inspired Analysis with Relative Memory Change ---")
        ...
        print("\n--- Starting Data-Adapted with Global Memory Change ---")
        compute_spectrum_figure(
            run_key='sub-01_Repos',
...
compute_spectrum_figure_replication_inspired_params_rev_mem_job = jobtools.Job(precomputedir, 'compute_spectrum_figure_replication_inspired_rev_mem', spectrum_params_replication, compute_spectrum_figure)
jobtools.register_job(compute_spectrum_figure_replication_inspired_params_rev_mem_job)

compute_spectrum_figure_rev_mem_job = jobtools.Job(precomputedir, 'compute_spectrum_figure_rev_mem', spectrum_params, compute_spectrum_figure)
jobtools.register_job(compute_spectrum_figure_rev_mem_job)


if __name__ == "__main__":

    plot_correlation_EEG_memory_reversed_memory_score(ranked_data=False, replication_inspired_analysis=False)
    plot_correlation_EEG_memory_reversed_memory_score(ranked_data=False, replication_inspired_analysis=True)
