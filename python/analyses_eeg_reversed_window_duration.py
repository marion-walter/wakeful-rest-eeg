...
from analyses_eeg_params import spectrum_params, spectrum_params_replication
...
def compute_spectrum(run_key, **p):
    ...
    **p : dict
    ...
    if p == spectrum_params_replication:
    ...
    # ---- Optional (density, less standard) ----
    ...
    for condition in ['Repos', 'Distraction']:
        ...
        for base in subject_keys:  
            ...
            if replication_inspired == True:
                ...
                if compute_spectrum_replication_inspired_params_rev_win_job.is_job_done(key):
                    subject_list.append(key)
            else:
                # print(f"Checking if job done for original params: {key}")
                if compute_spectrum_rev_win_job.is_job_done(key):
                    subject_list.append(key)
        ...
        for subject in subject_list:
            if replication_inspired == True:
                ...
                ds = compute_spectrum_replication_inspired_params_rev_win_job.get(subject)
            else:
                # print(f"Loading original spectrum for subject: {subject}")
                ds = compute_spectrum_rev_win_job.get(subject)
        ...
        for order in ['both']:        
            ...
            for band in frequency_bands:
                    ...
                    if replication_inspired == True:
                        ...
                        print("Using original params for correlation analysis.")
                        df_o_d = df_all.loc[order_mask & (df_all['Frequency_band']== band)]
                        df_memory_sub = df_memory[(df_memory['condition'] == condition)]
                        if order != 'both':
                            df_o_d_order = df_o_d[df_o_d['order'] == order]
                    ...
                    if replication_inspired == True:
                        df_results.to_csv(figures_path / 'eeg_correlations_results_replication_inspired_reversed_window.csv', index=False)
                    if replication_inspired == False:
                        df_results.to_csv(figures_path / 'eeg_correlations_results_reversed_window.csv', index=False)
                    ...
                    if replication_inspired == True:
                        ...
                        if rank_data == True:
                            update_figure_axes('PSD (window duration = 20 sec), 6 channels', band, axes, x_ranked, y_ranked, slope, intercept, r_value, p_value, r_channels, p_channels, info, rank_data = True, replication_inspired = True)
                            name = f"Order-{order}_{band}_Cond-{condition}_ranked.png"
                            folder = figures_path / 'unused_figures' / 'reversed_window_duration' / 'long_window_replication_inspired_analysis' / 'ranked'
                        else:
                            update_figure_axes('PSD (window duration = 20 sec), 6 channels', band, axes, x, y, slope, intercept, r_value, p_value, r_channels, p_channels, info, rank_data = False, replication_inspired = True)
                            if band == 'Slow (0.3 - 1 Hz)' and condition == 'Rest':
                                name = f"Supplementary_Figure_9B.png"
                                folder = figures_path 
                            else:
                                name = f"Order-{order}_{band}_Cond-{condition}.png"
                                folder = figures_path / 'unused_figures' / 'reversed_window_duration' / 'long_window_replication_inspired_analysis' / 'absolute'
                            
                    else:
                        if rank_data == True:
                            update_figure_axes('PSD (window duration = 4 sec), 62 channels', band, axes, x_ranked, y_ranked, slope, intercept, r_value, p_value, r_channels, p_channels, info, rank_data = True, replication_inspired = False)
                            name = f"Order-{order}_{band}_Cond-{condition}_ranked.png"
                            folder = figures_path / 'unused_figures' / 'reversed_window_duration' / 'short_window_data_specific_analysis' / 'ranked'
                        else:
                            update_figure_axes('PSD (window duration = 4 sec), 62 channels', band, axes, x, y, slope, intercept, r_value, p_value, r_channels, p_channels, info, rank_data = False, replication_inspired = False)
                            name = f"Order-{order}_{band}_Cond-{condition}.png"
                            folder = figures_path / 'unused_figures' / 'reversed_window_duration' / 'short_window_data_specific_analysis' / 'absolute'
                    ...
                    if replication_inspired:
                        save_figure(fig, name, compute_spectrum_replication_inspired_params_rev_win_job, folder, plotly=False, with_job=True)
                        save_figure(fig, name, compute_spectrum_replication_inspired_params_rev_win_job, folder, plotly=False, with_job=False)
                    else:
                        save_figure(fig, name, compute_spectrum_figure_rev_win_job, folder, plotly=False, with_job=True)
                        save_figure(fig, name, compute_spectrum_figure_rev_win_job, folder, plotly=False, with_job=False)
...
def compute_spectrum_all_reversed_window():
    ...
    jobtools.compute_job_list(compute_spectrum_replication_inspired_params_rev_win_job, run_keys, force_recompute=False, engine = 'loop')
    if spectrum_params_replication['rank_data'] == True:
        print("spectrum for replication-inspired analysis with reversed window duration (20 sec) on ranked data computed")
    else:
        print("spectrum for replication-inspired analysis with reversed window duration (20 sec) on real data computed")

    jobtools.compute_job_list(compute_spectrum_rev_win_job, run_keys, force_recompute=False, engine = 'loop')
    if spectrum_params['rank_data'] == True:
        print("spectrum for data-adapted analysis with reversed window duration (4 sec) on ranked data computed")
    else:
        print("spectrum for data-adapted analysis with reversed window duration (4 sec) on real data computed")


def plot_correlation_EEG_memory_reversed_window_duration(ranked_data, replication_inspired_analysis):

    temp_spectrum_params_replication = spectrum_params_replication.copy()
    temp_spectrum_params = spectrum_params.copy()
    ...
    if replication_inspired_analysis:
        print("\n--- Starting Replication-Inspired Analysis with Long Window ---")
        ...
        print("\n--- Starting Data-Adapted Analysis with Short Window ---")
        compute_spectrum_figure(
            run_key='sub-01_Repos',
...
compute_spectrum_replication_inspired_params_rev_win_job = jobtools.Job(precomputedir, 'compute_spectrum_replication_inspired_rev_win', spectrum_params_replication, compute_spectrum)
jobtools.register_job(compute_spectrum_replication_inspired_params_rev_win_job)

compute_spectrum_rev_win_job = jobtools.Job(precomputedir, 'compute_spectrum_rev_win', spectrum_params, compute_spectrum)
jobtools.register_job(compute_spectrum_rev_win_job)

compute_spectrum_figure_replication_inspired_params_rev_win_job = jobtools.Job(precomputedir, 'compute_spectrum_figure_replication_inspired_rev_win', spectrum_params_replication, compute_spectrum_figure)
jobtools.register_job(compute_spectrum_figure_replication_inspired_params_rev_win_job)

compute_spectrum_figure_rev_win_job = jobtools.Job(precomputedir, 'compute_spectrum_figure_rev_win', spectrum_params, compute_spectrum_figure)
jobtools.register_job(compute_spectrum_figure_rev_win_job)

if __name__ == "__main__":

    plot_correlation_EEG_memory_reversed_window_duration(ranked_data=False, replication_inspired_analysis=False)
